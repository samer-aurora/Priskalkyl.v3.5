...
window.App = {
  ...
  onLogin(user) {
    ...
    // Visa användarinfo
    const name = user.email?.split('@')[0] || 'Kund';
...
window.Dashboard = {
  ...
  _renderFilters() {
    ...
    document.getElementById('filter-pills').innerHTML = statuses.map(s => {
  ...
  _renderProjects() {
    const filtered = App.filterStatus === 'all'
      ? App.projects
    ...
    document.getElementById('projects-grid').innerHTML = filtered.map(p => {
      ...
      const steps = ['foranmalan','medgivande','fardiganmalan','driftsattning'];
      ...
      <div class="project-card" onclick="App.showDetail('${p.projectId}')">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:12px">
          ...
          <span class="badge badge-${si.cls}">${si.label}</span>
        </div>
      ...
      <div style="display:flex;flex-direction:column;gap:16px">
        <div class="card">
          ...
          <div class="install-flow">
            ${['foranmalan','medgivande','fardiganmalan','driftsattning'].map(k => {
              const done = !!flow[k];
...
window.CalcUI = {
  ...
  exportPDF() {
    UI.toast('PDF-export — implementeras i nästa version', 'success');
  },
...
/* ════════════════════════════════════════════════════════════
   INIT
   Bind login-form events och starta auth-listener
════════════════════════════════════════════════════════════ */
